...
// Convert into an NES
impl Into<Nes> for INes {
    fn into(self) -> Nes { Nes::new_from_rom(self.prg_rom()) }
}
...
mod tests {
    ...
    #[test]
    fn nes() {
        let nes: Nes = load_test().into();
        assert_eq!(nes.mem.peek_at(0xC000), 0x4C);
    }
}
